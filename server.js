...
// ===== Session (compatível com browser no Render) =====
...
app.use(
  session({
    ...
    cookie: {
      httpOnly: true,
      secure: false,           // ✅ evita sumir sessão no teste
      sameSite: "lax",         // ✅ funciona abrindo no browser normal
...
app.post("/api/test-token", async (req, res) => {
  try {
    ...
    let accounts = [];
    try {
      accounts = await fetchBusinessAccounts(token);
    } catch (_) {}

    // fallback basic
    if (!accounts.length) {
      try {
        accounts = await fetchBasicAccount(token);
      } catch (_) {}
    }
...
// ===== Suggestions (Inteligência) =====
app.post("/api/suggestions", requireAuth, async (req, res) => {
  try {
    const { igId } = req.body || {};
    const acc = (req.session.accounts || []).find((a) => a.id === igId) || (req.session.accounts || [])[0];

    const nicheHint = "conteúdo do Instagram";
    const out = await generateSuggestions({ clients: aiClients, nicheHint });
...
// ===== Planner (Planejador 30 dias) =====
app.post("/api/generate", requireAuth, async (req, res) => {
  try {
    const { igId, goal, tone } = req.body || {};
    const acc = (req.session.accounts || []).find((a) => a.id === igId) || (req.session.accounts || [])[0];
    if (!acc) return res.status(400).json({ error: "Nenhuma conta disponível" });

    const niche = "negócios locais"; // você pode evoluir depois para puxar do Mongo
